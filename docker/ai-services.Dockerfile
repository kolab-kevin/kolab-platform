...
COPY apps/ai-services/package.json ./apps/ai-services/
...
RUN pnpm turbo build --filter=@kolab/ai-services...
...
COPY --from=builder --chown=kolab:kolab /app/apps/ai-services/dist ./apps/ai-services/dist
COPY --from=builder --chown=kolab:kolab /app/apps/ai-services/package.json ./apps/ai-services/package.json
