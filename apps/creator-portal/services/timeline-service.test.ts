...
import { fetchSessionTimeline } from '@/services/timeline-service';
...
describe('fetchSessionTimeline', () => {
  it('returns mock timeline when mock mode is enabled', async () => {
    const result = await fetchSessionTimeline(MOCK_LIVE_SESSION_ID);
  ...
  it('fetches live timeline when mock mode is disabled', async () => {
    ...
    expect(apiGetMock).toHaveBeenCalledWith(`/api/live/sessions/${MOCK_LIVE_SESSION_ID}/timeline`);
  });
  ...
  it('throws DashboardApiError on unauthorized responses', async () => {
    ...
    await expect(fetchSessionTimeline(MOCK_LIVE_SESSION_ID)).rejects.toBeInstanceOf(
      DashboardApiError,
    );
  });
