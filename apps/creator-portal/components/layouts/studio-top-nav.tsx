...
export function StudioTopNav({ onToggleSidebar }: StudioTopNavProps) {
  const { user, logout } = useAuth();
  const { organizations, activeOrganization, setActiveOrganizationId, creatorProfile } =
    useOrganization();
  ...
  return (
    <header className="border-border/60 bg-background/80 sticky top-0 z-30 border-b backdrop-blur-md">
      <div className="flex h-14 items-center gap-3 px-4 lg:px-6">
        <button
          type="button"
          className="text-muted-foreground hover:text-foreground inline-flex h-9 w-9 items-center justify-center rounded-md border border-white/10 lg:hidden"
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            type="button"
            className="text-muted-foreground hover:text-foreground relative inline-flex h-9 w-9 items-center justify-center rounded-md border border-white/10"
          ...
          <div className="relative">
            <button
              type="button"
              className="hover:bg-accent/40 flex items-center gap-2 rounded-md border border-white/10 px-2 py-1.5 text-sm"
            ...
            >
              <span className="bg-primary/20 text-primary flex h-8 w-8 items-center justify-center rounded-full text-xs font-semibold">
                {(creatorProfile.displayName ?? user?.email ?? 'C').slice(0, 1).toUpperCase()}
              </span>
            ...
            {menuOpen ? (
              ...
              >
                ...
                <Link href="/studio/profile" className="hover:bg-accent/40 block px-3 py-2 text-sm">
                  Profile
                </Link>
                <Link
